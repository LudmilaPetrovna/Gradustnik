...
//also compatible with ds18b20 api
...
int DHT_read(int gpio, float *temperature, float *humidity){
...
int DHT_read(int gpio, float *temperature, float *humidity){
	...
	*humidity=(double)rand()/100.0;
	trace("reading DHT22 at pin %d: %.2f deg / %.2f%%",gpio,*temperature,*humidity);
	return 0;
}
